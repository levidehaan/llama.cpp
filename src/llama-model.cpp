            ...
            case LLM_ARCH_DREAM:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        const uint32_t n_head_i    = hparams.n_head(i);
                        const uint32_t n_head_kv_i = hparams.n_head_kv(i);
                        const int64_t  n_embd_q    = n_embd_head_k * n_head_i;
                        const int64_t  n_embd_kv_i = n_embd_head_k * n_head_kv_i;

                        layer.attn_norm = create_tensor(tn(LLM_TENSOR_ATTN_NORM, "weight", i), {n_embd}, 0);

                        layer.wq = create_tensor(tn(LLM_TENSOR_ATTN_Q,   "weight", i), {n_embd, n_embd_q}, 0);
                        layer.wk = create_tensor(tn(LLM_TENSOR_ATTN_K,   "weight", i), {n_embd, n_embd_kv_i}, 0);
                        layer.wv = create_tensor(tn(LLM_TENSOR_ATTN_V,   "weight", i), {n_embd, n_embd_kv_i}, 0);
                        layer.wo = create_tensor(tn(LLM_TENSOR_ATTN_OUT, "weight", i), {n_embd_q, n_embd}, 0);

                        // optional bias tensors
                        layer.bq = create_tensor(tn(LLM_TENSOR_ATTN_Q,   "bias", i), {n_embd_q}, TENSOR_NOT_REQUIRED);
                        layer.bk = create_tensor(tn(LLM_TENSOR_ATTN_K,   "bias", i), {n_embd_kv_i}, TENSOR_NOT_REQUIRED);
                        layer.bv = create_tensor(tn(LLM_TENSOR_ATTN_V,   "bias", i), {n_embd_kv_i}, TENSOR_NOT_REQUIRED);
